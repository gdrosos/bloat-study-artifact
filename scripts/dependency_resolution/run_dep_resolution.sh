...
if [ "$SUBSET" = "subset" ]; then
    ...
    python scripts/dependency_resolution/clone_repos.py -t $GITHUB_TOKEN  -f $DEST_DIR/python_projects.csv -d $DEST_DIR
    python scripts/dependency_resolution/dependency_resolver.py -f $DEST_DIR/python_projects.csv -d $DEST_DIR
